...
class QuickAddPrompter {
    ...
    #updateQuickAddField(fieldName, fieldContent) {
        ...
        const updatedContent = [this.#quickAddApi.variables[fieldName], fieldContent].flat().filter(field => field !== undefined);
    ...
    async suggestBlockToParse() {
        ...
        await this.#quickAddApi.infoDialog("What kind of info do you want to enter?");
    ...
    async getMonsterDescription() {
        ...
        this.#quickAddApi.variables = Object.assign(this.#quickAddApi.variables, monsterDescription);

        return BlockWriter.writeDescriptionBlock(monsterDescription);
    }

    async getMonsterActions() {
        const attackText = await this.#quickAddApi.wideInputPrompt("Monster Attacks (including \[Special Trigger\])?");
        ...
        return {
            ...
            triggeredActions: BlockWriter.writeTriggeredAttacksBlock(updatedAttacks.triggeredActions)
        };
    }

    async getMonsterTraits() {
        const traitText = await this.#quickAddApi.wideInputPrompt("Monster Traits?");
    ...
    async getMonsterTriggeredActions() {
        const attackText = await this.#quickAddApi.wideInputPrompt("Monster Triggered Attacks?");
        const attackParser = new BlockParser(attackText);
    ...
    async getMonsterNastierTraits() {
        const traitText = await this.#quickAddApi.wideInputPrompt("Monster Nastier Specials?");
        const traitParser = new BlockParser(traitText);
    ...
    async getMonsterDefenses() {
        ...
        this.#quickAddApi.variables = Object.assign(this.#quickAddApi.variables, monsterDefenses);
